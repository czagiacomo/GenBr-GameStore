...
public class ProductController {
	...
	public ResponseEntity<List<Product>> getByGameTitle(@PathVariable String gameTitle){
		return ResponseEntity.ok(productRepository.findAllByGameTitleContainingIgnoreCase(gameTitle));
	}
	...
	public ResponseEntity<Product> put(@Valid @RequestBody Product product){
		return productRepository.findById(product.getId()).map(response -> ResponseEntity.status(HttpStatus.OK)
				.body(productRepository.save(product))).orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
	}
